<template>
    <section class="UserEdit">
        <h3>Profile</h3>
        <validation-observer ref="observer" v-slot="{ invalid }">
            <form @submit.prevent="handleSubmit" method="POST" refs="form">
                <validation-provider
                    v-slot="{ errors }"
                    name="Name"
                    rules="required|max:50"
                >
                    <v-text-field
                        v-model="name"
                        :counter="50"
                        :error-messages="errors"
                        label="Name"
                        required
                    ></v-text-field>
                </validation-provider>

                <v-btn
                    class="mr-4 primary"
                    type="submit"
                    :disabled="invalid"
                    :loading="loading"
                >
                    Update
                </v-btn>
            </form>
        </validation-observer>
        <template v-if="subcall === true">
            <user-address :useraddresses="addresses"></user-address>
        </template>
    </section>
</template>

<script>
import UserAddress from "./address.vue";
export default {
    components: {
        UserAddress
    },
    data: () => ({
        valid: true,
        name: "",
        subcall: false,
        addresses: [],
        loading: false
    }),

    methods: {
        async handleSubmit() {
            this.loading = true;
            this.$refs.observer.validate();
            const params = {
                name: this.name,
                _method: "put"
            };
            await axios
                .post("/app/user/" + this.$route.params.id, params)
                .then(response => {
                    this.loading = false;
                    const responseData = response.data;
                    this.$toasted.show("Updated.");
                })
                .catch(err => {
                    const response = err.response;
                    this.loading = false;
                    if (response.status == 422) {
                        let msg = "Server Data not present";
                        if (response.data) {
                            if (response.data.data.name) {
                                msg = JSON.stringify(
                                    response.data.data.name[0]
                                );
                            }
                            this.$toasted.show(msg);
                        }
                    } else {
                        this.$toasted.show("Please try again later!");
                    }
                });
        },
        async getData(id) {
            await axios
                .get("/app/user/" + id)
                .then(response => {
                    const data = response.data;
                    if (data.name) {
                        this.name = data.name;
                        this.addresses = data.addresses;
                        this.subcall = true;
                    } else {
                        this.$toasted.show("Server Error!");
                    }
                })
                .catch(err => {
                    this.$toasted.show("Server Error!");
                });
        }
    },
    created() {
        this.getData(this.$route.params.id);
    }
};
</script>

<style></style>
